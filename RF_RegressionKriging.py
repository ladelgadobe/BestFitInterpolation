...
class RegressionKrigingRFController:
    ...
    def _fit_rf_stage(self, progress_fn=None):
        ...
        cols = ["x", "y"] + list(self._covariate_names) + [self._target_name]
        train_df = self._train_df[cols].dropna().copy()

        X = train_df[["x", "y"] + list(self._covariate_names)].to_numpy(dtype=float)
        ...
        self._importance_df = pd.DataFrame(
            {
                "Variable": ["x", "y"] + list(self._covariate_names),
    ...
    def _run_rk_prediction(self, progress_fn=None):
        ...
        grid_cols = ["x", "y"] + list(self._covariate_names)
        grid_clean = self._grid_df[grid_cols].dropna().copy()
        X_grid = grid_clean[["x", "y"] + list(self._covariate_names)].to_numpy(dtype=float)
    ...
    def _on_run_rk_cv_clicked(self):
        ...
        try:
            ...
            X_all = train_df[["x", "y"] + list(self._covariate_names)].to_numpy(dtype=float)

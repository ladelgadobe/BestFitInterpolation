...
def rf_interpolation(
    ...
    covariate_columns = [
        c for c in covariate_columns if c not in (x_col, y_col)
    ]

    cols_needed = [x_col, y_col] + covariate_columns + [target_column]
    train_df = points_df[cols_needed].dropna().copy()

    if train_df.empty:
        raise ValueError("No valid training rows after dropping NA in predictors/target.")

    X = train_df[[x_col, y_col] + covariate_columns].to_numpy(dtype=float)
    ...
    feature_names = [x_col, y_col] + covariate_columns

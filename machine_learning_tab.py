...
- Lower triangular correlation heatmap using viridis
...
class MachineLearningTabController:
    ...
    def _rf_lccc(self, obs, pred):
        ...
        var_o = float(np.var(o, ddof=1)) if len(o) > 1 else float("nan")
        var_p = float(np.var(p, ddof=1)) if len(p) > 1 else float("nan")
        cov_op = float(np.cov(o, p, ddof=1)[0, 1]) if len(o) > 1 else float("nan")
    ...
    def _on_resample_covariates(self):
        ...
        for i in range(total):
            ...
            out_path = os.path.join(self._tmp_dir, f"{out_name}.tif")
            ...
            resampled_layer = QgsRasterLayer(out_path, out_name, "gdal")
    ...
    def _on_apply_standardization_clicked(self):
        ...
        for i in range(total):
            ...
            std_layer = QgsRasterLayer(out_path, out_name, "gdal")
    ...
    def _draw_corr_plot(self, corr_matrix, var_names, sample_count, fig, canvas):
        ...
        im = ax.imshow(matrix_to_plot, cmap="viridis", vmin=-1.0, vmax=1.0)
    ...
    def _set_rf_info_icons(self):
        """Set RF info-button icons using the shared plugin info.png file."""
        try:
        ...
        try:
            ...
            for name in ["btnInfoRFSearchK", "btnInfoRFSearchIter"]:
                btn = getattr(self.dlg, name, None)
                if btn is None:
                    continue
    ...
    def _ensure_output_dir_for_rf(self):
        """
        Create BestFitInterpolation folder inside the project directory (same as deterministic),
        used for RF rasters.
        """
    ...
    def _build_points_dataframe_for_rf(self):
        ...
        if not cov_names:
            self._show_warning_message(
                "No covariates",
                "Please load at least one covariate raster before running RF interpolation.",
            )
            return None, None, None
        ...
        raster_crs = raster_layers[0].crs()
        point_crs = points_layer.crs()
        if raster_crs.isValid() and point_crs.isValid() and raster_crs != point_crs:
        ...
        return points_df, attr_name, cov_names
    ...
    def _build_grid_dataframe_for_rf(
        ...
        raster_layers = [self.covariate_layers.get(name) for name in cov_names]
        ...
        if raster_crs.isValid() and poly_crs.isValid() and raster_crs != poly_crs:
    ...
    def _on_run_rf_cross_validation(self):
        ...
        try:
            ...
            cols = ["x", "y"] + list(cov_names) + [target_name]
            ...
            X_all = train_df[["x", "y"] + list(cov_names)].to_numpy(dtype=float)
    ...
    def _set_svm_info_icons(self):
        """Set SVM info-button icons using the shared plugin info.png file."""
        try:
            from qgis.PyQt.QtGui import QIcon
            from qgis.PyQt.QtCore import QSize
        except Exception:
        ...
        try:
            ...
            for name in names:
                ...
                try:
                    ...
                    btn.setToolButtonStyle(Qt.ToolButtonIconOnly)
                    btn.setMinimumSize(QSize(18, 18))
                    btn.setMaximumSize(QSize(18, 18))
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def _on_run_svm_interpolation(self):
        ...
        if train_mae is not None:
            w = getattr(self.dlg, "valSVMMAE", None)
            if w is not None and hasattr(w, "setText"):
                w.setText(f"{train_mae:.2f}")
        if train_rmse is not None:
            w = getattr(self.dlg, "valSVMRMSE", None)
            if w is not None and hasattr(w, "setText"):
                w.setText(f"{train_rmse:.2f}")

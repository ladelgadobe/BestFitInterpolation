...
class FrameworkTabController(QObject):
    ...
    def _show_canvas_context_menu(self, canvas: FigureCanvas, get_figure_fn, save_prefix: str, zoom_title: str, pos) -> None:
        ...
        if chosen == act_save and fig is not None:
    ...
    def _open_framework_covariates_dialog(self) -> None:
        ...
        try:
            ...
            btn_extract = QPushButton("Extract Covariates")
    ...
    def _validation_metrics_table_html(self) -> str:
        ...
        return (
            "<table>"
            "<tr><th>Rank</th><th>Method</th><th>Pearson r</th><th>MAE</th><th>R²</th><th>RMSE %</th><th>RMSE</th><th>LCCC</th></tr>"
            f"{body}"
            "</table>"
        )
    ...
    def _populate_validation_table(self, rows: List[Dict[str, Any]]) -> None:
        ...
        headers = ["Rank", "Method", "Pearson r", "MAE", "R²", "RMSE %", "RMSE", "LCCC"]
    ...
    def _lccc(obs, pred) -> float:
        ...
        cov_op = float(np.cov(o, p, ddof=1)[0, 1])
    ...
    def _run_ok_framework_cv(self, x: np.ndarray, y: np.ndarray, z: np.ndarray) -> np.ndarray:
        model_name = str(self.state.__dict__.get("variogram_model") or "Spherical")
        dmax = float(np.nanmax(self._pairwise_distances(x, y))) if z.size > 1 else 1.0
        cutoff = 0.5 * dmax
        lagw = max(self._nearest_neighbor_dist(x, y), 1e-9)
